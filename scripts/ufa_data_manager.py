...
from backend.sql_database import SQLDatabase, get_db
...
class UFADataManager:
    ...
    def import_from_api(self, years: Optional[List[int]] = None, clear_existing: bool = True) -> Dict[str, int]:
        """
    ...
    def _clear_database(self):
        ...
        tables = ["player_game_stats", "player_season_stats", "team_season_stats", "games", "players", "teams"]
        for table in tables:
            try:
    ...
    def _import_teams_from_api(self, teams_data: List[Dict[str, Any]], years: List[int]) -> int:
        ...
        for team in teams_data:
            try:
                ...
                logger.warning(f"Failed to import team {team.get('name', 'unknown')}: {e}")
    ...
    def _import_players_from_api(self, players_data: List[Dict[str, Any]]) -> int:
        ...
        for player in players_data:
            try:
                ...
                logger.warning(f"Failed to import player {player.get('fullName', 'unknown')}: {e}")
    ...
    def _import_games_from_api(self, games_data: List[Dict[str, Any]]) -> int:
        ...
        for game in games_data:
            try:
                ...
                logger.warning(f"Failed to import game {game.get('gameID', 'unknown')}: {e}")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("")
        print("Examples:")
        print("  python ufa_data_manager.py import-api          # Import all years (2012-2025, excluding 2020)")
        print("  python ufa_data_manager.py import-api 2023     # Import only 2023")
        print("  python ufa_data_manager.py import-api 2022 2023 2024  # Import specific years")
        sys.exit(1)
