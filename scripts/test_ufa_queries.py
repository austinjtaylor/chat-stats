...
from backend.sql_database import SQLDatabase
import logging
...
def test_database():
    ...
    logger.info("\n1. Available seasons:")
    result = db.execute_query("SELECT DISTINCT season FROM player_season_stats ORDER BY season")
    ...
    """)
    for r in result:
        expected = r['total_goals'] + r['total_assists'] + r['total_blocks'] - \
                  r['total_throwaways'] - r['total_stalls'] - r['total_drops']
        correct = expected == r['calculated_plus_minus']
        logger.info(f"   {r['name']}: {r['calculated_plus_minus']} "
                   f"(G:{r['total_goals']} A:{r['total_assists']} B:{r['total_blocks']} "
                   f"T:{r['total_throwaways']} S:{r['total_stalls']} D:{r['total_drops']}) "
                   f"{'✓' if correct else '✗'}")
    tests.append(("Plus/minus calculation", all(r['calculated_plus_minus'] > 0 for r in result)))
    ...
    """)
    for r in result:
        logger.info(f"   {r['name']}: {r['completion_percentage']:.1f}% "
