...
def validate_price_id(price_id: str) -> None:
    ...
    if price_id not in valid_ids:
        raise HTTPException(status_code=400, detail="Price ID not recognized")
...
def validate_redirect_url(url: str) -> None:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid redirect URL format")

    # Must be HTTP or HTTPS
    if parsed.scheme not in ["http", "https"]:
        raise HTTPException(
            status_code=400, detail="Redirect URL must use HTTP or HTTPS"
        )

    # localhost must use HTTP in development
    if "localhost" in parsed.netloc and parsed.scheme != "http":
        raise HTTPException(status_code=400, detail="localhost URLs must use HTTP")

    # Production domains must use HTTPS
    if "localhost" not in parsed.netloc and parsed.scheme != "https":
        raise HTTPException(status_code=400, detail="Production URLs must use HTTPS")
    ...
    if not allowed:
        raise HTTPException(
            status_code=400, detail=f"Redirect URL domain not allowed: {netloc}"
        )
