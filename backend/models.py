...
class Team(BaseModel):
    ...
    division_id: Optional[str] = None  # Division ID
    division_name: Optional[str] = None  # Division name
...
class Player(BaseModel):
    ...
    team_id: Optional[str] = None  # UFA teamID
    active: bool = True  # Whether player is active
    year: Optional[int] = None  # Year for team association
    jersey_number: Optional[int] = None  # Player's jersey number
...
class Game(BaseModel):
    ...
    start_timestamp: Optional[datetime] = None  # Game start timestamp
    start_timezone: Optional[str] = None  # Start timezone
    streaming_url: Optional[str] = None  # Streaming URL
    update_timestamp: Optional[datetime] = None  # Last update timestamp
    week: Optional[str] = None  # Week identifier
    location: Optional[str] = None  # Game location
    year: int  # Four digit year
...
class PlayerGameStats(BaseModel):
    ...
    recorded_pulls_hangtime: Optional[int] = None
    ...
    def calculated_plus_minus(self) -> int:
        ...
        return self.goals + self.assists + self.blocks - self.throwaways - self.stalls - self.drops
...
class PlayerSeasonStats(BaseModel):
    ...
    total_recorded_pulls_hangtime: Optional[int] = None
    ...
    # Calculated fields
    calculated_plus_minus: Optional[int] = None
    completion_percentage: Optional[float] = None
    
    @property
    def plus_minus(self) -> int:
        """Calculate UFA plus/minus: goals + assists + blocks - throwaways - stalls - drops"""
        return self.total_goals + self.total_assists + self.total_blocks - self.total_throwaways - self.total_stalls - self.total_drops
...
class TeamSeasonStats(BaseModel):
    ...
    losses: int = 0  # Number of losses
    ties: int = 0  # Number of ties
    standing: Optional[int] = None  # Team's standing
    division_id: Optional[str] = None  # Division ID
    division_name: Optional[str] = None  # Division name
