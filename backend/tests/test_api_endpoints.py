...
def mock_stats_system():
    ...
    with patch('app.stats_system') as mock:
        ...
        mock.get_recent_games.return_value = [{"game_id": "test-game", "home_team": "Team A", "away_team": "Team B"}]
        mock.get_database_info.return_value = {"players": "table info", "teams": "table info"}
...
class TestQueryEndpoint:
    ...
    def test_query_endpoint_success(self, test_client, mock_stats_system):
        ...
        mock_stats_system.query.assert_called_once_with("Who are the top scorers?", "test-session-123")
...
class TestDataImportEndpoint:
    ...
    def test_import_data_success(self, test_client, mock_stats_system):
        ...
        with patch('app.os.path.exists', return_value=True):
            response = test_client.post("/api/data/import?file_path=/test/data.json&data_type=json")
            ...
            mock_stats_system.import_data.assert_called_once_with("/test/data.json", "json")
...
class TestCORS:
    ...
    def test_options_request(self, test_client):
        ...
        assert response.status_code == 405  # Method not allowed, but CORS should still work