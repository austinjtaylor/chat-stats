...
class DatabaseStatsService:
    ...
    def get_stats_summary(self) -> dict[str, Any]:
        ...
        summary["seasons"] = self._get_available_seasons()

        # Get team standings
        if summary["seasons"]:
            current_year = int(summary["seasons"][0]) if summary["seasons"] else 2025
            summary["team_standings"] = self._get_team_standings(current_year)
    ...
    def _get_available_seasons(self) -> list[str]:
        ...
        try:
            ...
            return [str(row["year"]) for row in seasons_result]
        except Exception as e:
            print(f"Could not get seasons: {e}")
            return []

    def _get_team_standings(self, current_year: int) -> list[dict[str, Any]]:
        """
        Get team standings for all teams.

        Args:
            current_year: Most recent season year
